...
* **Recognition** : 문자 인식 모델 (관련 코드는 root 폴더에)
* **Detection** : 문자 영역 탐지 모델 (관련 코드는 craft 폴더에)
...
├── ...
└── craft                       # detection에 관련된 코드들이 담겨 있습니다. 관련 설명은 craft 폴더 내의 README를 참고해주세요.
    ├── train.py
    ├── eval.py
    └── ...
```
...
1. **이미지 자르기** : JSON 파일에 기록된 좌표값을 기준으로 포스터 내 텍스트가 위치한 이
미지를 문자 인식 모델에 적합한 형태로 잘라냄
2. **글머리 기호 데이터 증강** : 데이터에 포함된 글머리 기호 이미지를 활용하여, 색상, 크기, 위치를 변
경하며 데이터를 증강
3. **데이터셋 분할** : 포스터 이미지 데이터와 증강된 글머리 기호 데이터를 합쳐 학습 데이
터셋, 검증 데이터셋, 테스트 데이터셋을 6:2:2의 비율로 분할
4. **레이블링** : 이미지 파일과 JSON파일을 참고하여 정답 레이블 형식에 맞게 구축
5. **데이터셋 포맷 변환** : 인공지능 모델이 요구하는 데이터 포맷으로 변환
6. **모델 학습** : recognition_train.ipynb 파일에서 사용한 동일한 파라미터로 학습

### **Detection**

1. **글머리 기호 데이터 증강** : 데이터에 포함된 글머리 기호 이미지를 활용하여, 색상, 크기, 위치를 변
경하며 데이터를 증강
2. **데이터셋 분할** : 포스터 이미지 데이터와 증강된 글머리 기호 데이터를 합쳐 학습 데이
터셋, 검증 데이터셋, 테스트 데이터셋을 6:2:2의 비율로 분할
3. **레이블링** : 이미지 파일과 JSON파일을 참고하여 정답 레이블 형식에 맞게 구축
4. **모델 학습** : 인공지능 모델이 요구하는 데이터 포맷으로 변환
